...
public class BoardImpl implements Board {
	...
	public void move(Move m) throws IllegalMoveException {
		...
		if (piece!=null && !isLegalFor(m, piece.getColor()))
	...
	public Color checkForVictory() {
		Point kingLoc = getPieceLocations(Piece.WhiteKing).get(0);
		
		//king in edge castle: white won
		if (isEscapeCastle(kingLoc))
			return Color.White;
		...
		//get all the location of black attackers
		...
		List<Point> blackList = this.getPieceLocations(Piece.BlackWarrior);
		if(blackList.size() == 2)
		{
			return Color.White;
		}
	...
	public boolean isLegalFor(Move m, Color color) {
		...
		if (!getPieceAt(m.getSource()).isKing()) {
			if (m.getDestination().x == 1 || m.getDestination().x == boardSize)
				return m.getDestination().y != m.getDestination().x;
